...
double steer_control_get_angle (const steer_control_t *con) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        fuzzy_val[i] = 3.0 / fmax(dist[i], 3.0);
    }

    /* left * angle -40 */
    fire_str[0] = fuzzy_and(fuzzy_val[0],
            fuzzy_not(6.0/fmax (dist[2], 6.0)));


    /* mid and ((left and right) or (not left and not right)) */
    fire_str[1] = fuzzy_and(fuzzy_val[1],
            fuzzy_or(fuzzy_and(fuzzy_val[0], fuzzy_val[2]),
                fuzzy_and(fuzzy_not(fuzzy_val[0]), fuzzy_not(fuzzy_val[2]))));

    /* right * angle 40 */
    fire_str[2] = fuzzy_and(fuzzy_val[2],
            fuzzy_not(6.0 / fmax (dist[2], 6.0)));
